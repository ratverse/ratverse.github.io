...
window.addEventListener('load', async function() {
    if (window.ethereum) {
      ...
      try {
        ...
        nftContract =await new web3.eth.Contract(minersAbi, minersAddr)
...
function refreshData(){
    ...
    allEarnedReflectionRead(function(reflect){

        document.getElementsByClassName('numminers') = reflect
        for(var i=0;i<allnumminers.length;i++){
            if(allnumminers[i]){
                allnumminers[i].textContent=translateQuantity(reflect)
            }
        }
